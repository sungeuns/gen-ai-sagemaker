...
def parse_args():
    ...
    # path
    parser.add_argument("--base_model", type=str, default="TheBloke/Llama-2-7B-Chat-GGUF", help="Model id to use for training.")
...
def train(args):
    ...
    val_data = load_from_disk(os.path.join(data_path, "val"))
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        if save_merged_model: 
            ...
            # Saving merged model
            print(f'Merge model path: {save_path}')
            from peft import AutoPeftModelForCausalLM
            os.makedirs(save_path, exist_ok=True)
            
            print(f'checkpoint list: {os.listdir(output_dir)}')
            del model
            model = AutoPeftModelForCausalLM.from_pretrained(output_dir, low_cpu_mem_usage=True, torch_dtype=data_type)
            merged_model = model.merge_and_unload()
            merged_model.save_pretrained(save_path, safe_serialization=True)
            print('Success to save merged model !!')
        else:
            print(f'Save LoRA model: {save_path}') 
            trainer.model.save_pretrained(save_path)

        tokenizer.save_pretrained(save_path)
        print('Success to save tokenizer !!')
